...
- Чтобы роль nginx начала корректно проксировать запросы на порт 80, мы добавили определение переменных
  nginx_sites: в файл  ansible/roles/app/vars main.yml

Основное ДЗ Molecule*

- Установили основный инструменты (molecule, testinfram python-vagrant);
- Добавили несколько тестов в роль db;
- Модифицировали db/molecule/default/playbook.yml и прогнали все тесты с помощью molecule verify;
- Добавили тест TestInfra, который проверяет, что сокет слушает порт 27017;
- Добавили запуск ролей Ансибл в шаблоны пакер и проверили, что образы создаются корректно;
